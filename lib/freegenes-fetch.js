...
async function getSamples() {
  const res = await fetch('https://api.freegenes.org/samples/');
  return res.json();
}

async function getSample(id) {
  const res = await fetch(`https://api.freegenes.org/samples/${id}`);
  return res.json();
}

async function getFullSample(id) {
  const res = await fetch(`https://api.freegenes.org/samples/full/${id}`);
  return res.json();
}
...
module.exports = {
  ...
  getSamples, getSample, getFullSample
};